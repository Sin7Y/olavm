use crate::cpu::columns::{
    COL_AUX0, COL_AUX1, COL_IMM_VAL, COL_OP1, COL_OP1_IMM, COL_S_MSTORE, NUM_CPU_COLS,
};
...
pub(crate) fn eval_packed_generic<P: PackedField>(
    ...
    yield_constr.constraint(lv[COL_S_MSTORE] * lv[COL_OP1_IMM] * lv[COL_AUX0]);

    // when op1_imm is 0, aux0 is the imm
    yield_constr.constraint(
        lv[COL_S_MSTORE] * (P::ONES - lv[COL_OP1_IMM]) * (lv[COL_AUX0] - lv[COL_IMM_VAL]),
    );
}
...
pub(crate) fn eval_ext_circuit<F: RichField + Extendable<D>, const D: usize>(
    ...
    // when op1_imm is 1, offset must be 0
    let none_offset_cs =
        builder.mul_many_extension([lv[COL_S_MSTORE], lv[COL_OP1_IMM], lv[COL_AUX0]]);
