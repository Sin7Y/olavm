...
pub fn gen_memory_trace<F: RichField>(
    batch_cells: &Vec<Vec<MemoryTraceCell>>,
) -> [Vec<F>; memory::NUM_MEM_COLS] {
    let mut num_filled_row_len: usize = batch_cells.iter().map(|inner_vec| inner_vec.len()).sum();
    let num_padded_rows = if !num_filled_row_len.is_power_of_two() || num_filled_row_len < 2 {
        if num_filled_row_len < 2 {
            2
        } else {
    ...
    for (tx_idx, tx_cells) in batch_cells.iter().enumerate() {
        for c in tx_cells.iter() {
            ...
            trace[memory::COL_MEM_IS_WRITE][i] =
                F::from_canonical_u64(c.is_write.to_canonical_u64());
            trace[memory::COL_MEM_VALUE][i] = F::from_canonical_u64(c.value.to_canonical_u64());
            trace[memory::COL_MEM_DIFF_ADDR][i] =
                F::from_canonical_u64(c.diff_addr.to_canonical_u64());
            trace[memory::COL_MEM_DIFF_ADDR_INV][i] =
                F::from_canonical_u64(c.diff_addr_inv.to_canonical_u64());
            trace[memory::COL_MEM_DIFF_CLK][i] =
                F::from_canonical_u64(c.diff_clk.to_canonical_u64());
            trace[memory::COL_MEM_DIFF_ADDR_COND][i] =
                F::from_canonical_u64(c.diff_addr_cond.to_canonical_u64());
            trace[memory::COL_MEM_RW_ADDR_UNCHANGED][i] =
                F::from_canonical_u64(c.rw_addr_unchanged.to_canonical_u64());
            ...
            trace[memory::COL_MEM_REGION_HEAP][i] =
                F::from_canonical_u64(c.region_heap.to_canonical_u64());
            trace[memory::COL_MEM_RC_VALUE][i] =
                F::from_canonical_u64(c.rc_value.to_canonical_u64());
            let curr_is_heap = c.region_heap.to_canonical_u64() == 1;
            let last_is_not_heap = i > 0 && tx_cells[i - 1].region_heap.to_canonical_u64() == 0;
            trace[memory::COL_MEM_FILTER_LOOKING_RC][i] = if i == 0 {
                F::from_canonical_u64(0)
            } else if c.region_prophet.to_canonical_u64() == 1 {
                F::from_canonical_u64(0)
            ...
            trace[memory::COL_MEM_FILTER_LOOKING_RC_COND][i] = if c.region_heap.to_canonical_u64()
                == 1
                || c.region_prophet.to_canonical_u64() == 1
            {
                F::from_canonical_u64(1)
            } else {
                F::from_canonical_u64(0)
