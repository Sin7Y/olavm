...
impl<F: RichField + Extendable<D>, const D: usize> Stark<F, D> for MemoryStark<F, D> {
        ...
        P: PackedField<Scalar = FE>,
    {
        ...
        // let region_stack = P::ONES - lv[COL_MEM_REGION_HEAP] -
        // lv[COL_MEM_REGION_PROPHET]; let nv_region_stack = P::ONES -
        // nv[COL_MEM_REGION_HEAP] - nv[COL_MEM_REGION_PROPHET];
        ...
        // let op_mload =
        // P::Scalar::from_canonical_u64(OlaOpcode::MLOAD.binary_bit_mask());
        // let op_mstore =
        // P::Scalar::from_canonical_u64(OlaOpcode::MSTORE.binary_bit_mask());
        // let op_call =
        // P::Scalar::from_canonical_u64(OlaOpcode::CALL.binary_bit_mask());
        // let op_ret =
        // P::Scalar::from_canonical_u64(OlaOpcode::RET.binary_bit_mask());
        // let op_tload =
        // P::Scalar::from_canonical_u64(OlaOpcode::TLOAD.binary_bit_mask());
        // let op_tstore =
        // P::Scalar::from_canonical_u64(OlaOpcode::TSTORE.binary_bit_mask());
        // let op_sc_call =
        // P::Scalar::from_canonical_u64(OlaOpcode::SCCALL.binary_bit_mask());
        // let op_poseidon =
        ...
        // // constraint opcode and selector matches, selector is binary and
        // only one is // selected.
        // yield_constr.constraint((lv[COL_MEM_OP] - op_mload) *
        // lv[COL_MEM_S_MLOAD]); yield_constr.constraint((lv[COL_MEM_OP]
        // - op_mstore) * lv[COL_MEM_S_MSTORE]); yield_constr.
        // constraint((lv[COL_MEM_OP] - op_call) * lv[COL_MEM_S_CALL]);
        // yield_constr.constraint((lv[COL_MEM_OP] - op_ret) *
        // lv[COL_MEM_S_RET]); yield_constr.constraint((lv[COL_MEM_OP] -
        // op_tload) * lv[COL_MEM_S_TLOAD]); yield_constr.
        // constraint((lv[COL_MEM_OP] - op_tstore) * lv[COL_MEM_S_TSTORE]);
        // yield_constr.constraint((lv[COL_MEM_OP] - op_sc_call) *
        // lv[COL_MEM_S_SCCALL]); yield_constr.
        // constraint((lv[COL_MEM_OP] - op_poseidon) * lv[COL_MEM_S_POSEIDON]);
        // yield_constr.constraint((lv[COL_MEM_OP] - op_sstore) *
        // lv[COL_MEM_S_SSTORE]); yield_constr.
        // constraint((lv[COL_MEM_OP] - op_sload) * lv[COL_MEM_S_SLOAD]);
        // yield_constr.constraint((lv[COL_MEM_OP] - op_prophet) *
        // lv[COL_MEM_S_PROPHET]); yield_constr.constraint((P::ONES -
        // lv[COL_MEM_S_MLOAD]) * lv[COL_MEM_S_MLOAD]); yield_constr.
        // constraint((P::ONES - lv[COL_MEM_S_MSTORE]) * lv[COL_MEM_S_MSTORE]);
        // yield_constr.constraint((P::ONES - lv[COL_MEM_S_CALL]) *
        // lv[COL_MEM_S_CALL]); yield_constr.constraint((P::ONES -
        // lv[COL_MEM_S_RET]) * lv[COL_MEM_S_RET]); yield_constr.
        // constraint((P::ONES - lv[COL_MEM_S_TLOAD]) * lv[COL_MEM_S_TLOAD]);
        // yield_constr.constraint((P::ONES - lv[COL_MEM_S_TSTORE]) *
        // lv[COL_MEM_S_TSTORE]); yield_constr.constraint((P::ONES -
        // lv[COL_MEM_S_SCCALL]) * lv[COL_MEM_S_SCCALL]); yield_constr.
        // constraint((P::ONES - lv[COL_MEM_S_POSEIDON]) *
        // lv[COL_MEM_S_POSEIDON]); yield_constr.constraint((P::ONES -
        // lv[COL_MEM_S_SSTORE]) * lv[COL_MEM_S_SSTORE]); yield_constr.
        // constraint((P::ONES - lv[COL_MEM_S_SLOAD]) * lv[COL_MEM_S_SLOAD]);
        // yield_constr.constraint((P::ONES - lv[COL_MEM_S_PROPHET]) *
        // lv[COL_MEM_S_PROPHET]); yield_constr.constraint(
        //     P::ONES
        //         - lv[COL_MEM_S_MLOAD]
        //         - lv[COL_MEM_S_MSTORE]
        //         - lv[COL_MEM_S_CALL]
        ...
        //     (P::ONES - lv[COL_MEM_IS_RW]) * (P::ONES - lv[COL_MEM_S_PROPHET]
        // - lv[COL_MEM_S_MLOAD]), );
        // // constraint is_write
        // yield_constr.constraint(
        //     lv[COL_MEM_IS_WRITE]
        ...
        // yield_constr.constraint(P::ONES - region_stack - region_heap -
        // region_prophet); yield_constr.constraint(region_stack *
        // (P::ONES - region_stack)); yield_constr.
        // constraint(region_heap * (P::ONES - region_heap));
        // yield_constr.constraint(region_prophet * (P::ONES - region_prophet));
        // // make sure region addr is right
        // yield_constr.constraint(region_prophet * (p - addr -
        // diff_addr_cond)); yield_constr.constraint(region_heap * (p -
        // span - addr - diff_addr_cond));
        ...
        //         * region_stack
        //         * nv_region_stack
        //         * (P::ONES - nv_rw_addr_unchanged - nv_diff_addr *
        //           nv_diff_addr_inv),
        // );
        // yield_constr.constraint_transition(
        //     (P::ONES - nv[COL_MEM_TX_IDX] + lv[COL_MEM_TX_IDX])
        ...
        // yield_constr.constraint(
        //     region_prophet * nv_region_prophet * (nv_addr - addr) * (nv_addr
        // - addr - P::ONES), );
        // yield_constr.constraint(
        //     region_prophet * nv_region_prophet * (nv_addr - addr - P::ONES) *
        // nv_is_write, );
        ...
        // yield_constr.constraint((P::ONES - nv_is_write) * (nv_value -
        // value));
        ...
        // yield_constr.constraint((P::ONES - lv_filter_looking_rc_cond) *
        // region_heap); yield_constr.constraint(
        //     (P::ONES - lv_filter_looking_rc_cond) * region_prophet * (P::ONES
        // - is_write), );
    }
