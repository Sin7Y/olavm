...
impl<F: RichField + Extendable<D>, const D: usize> Stark<F, D> for CpuStark<F, D> {
        ...
        P: PackedField<Scalar = FE>,
    {
        ...
        for (dst, l_r, n_r) in izip!(&s_dsts[..REGISTER_NUM-1], &regs[..REGISTER_NUM-1], &n_regs[..REGISTER_NUM-1]) {
            yield_constr.constraint((P::ONES - *dst) * (*n_r - *l_r));
        }
        // fp
        yield_constr.constraint(lv[COL_S_RET] * (n_regs[REGISTER_NUM-1] - lv[COL_AUX0]) + (P::ONES - lv[COL_S_RET]) * (P::ONES - s_dsts[REGISTER_NUM-1]) * (n_regs[REGISTER_NUM-1] - regs[REGISTER_NUM-1]));

        // pc
        let pc_incr = (P::ONES - (lv[COL_S_JMP] + lv[COL_S_CJMP] + lv[COL_S_CALL] + lv[COL_S_RET])) * (lv[COL_PC] + P::ONES + lv[COL_OP1_IMM]);
        let pc_jmp = lv[COL_S_JMP] * lv[COL_OP1];
        let pc_cjmp = lv[COL_S_CJMP] * ((P::ONES - lv[COL_FLAG]) * (lv[COL_PC] + P::ONES + lv[COL_OP1_IMM]) + lv[COL_FLAG] * lv[COL_OP1]);
