...
>(
    ...
    ) in prover_data.lookup_rows.iter().enumerate()
    {
        ...
        let num_entries = BitwiseLookupGate::num_slots(&common_data.config);
        let num_lut_entries = BitwiseLookupTableGate::num_slots(&common_data.config);
        ...
        for lut_entry in 0..lut_len {
            ...
            let mul_target = Target::wire(row, BitwiseLookupTableGate::wire_ith_multiplicity(col));
