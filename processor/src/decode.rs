...
pub fn decode_raw_instruction(raw_inst_str: &str, imm_str: &str) -> Result<(String, u64), ParseIntError>{
    let mut step = NO_IMM_INSTRUCTION_LEN;
    let raw_inst = u32::from_str_radix(raw_inst_str.trim_start_matches("0x"), 16)?;
    debug!("raw_inst:{}", raw_inst);
    let op_code = Opcode::try_from((raw_inst >> OPCODE_FLAG_FIELD_BIT_POSITION) as u8).unwrap();
    ...
    match op_code {
        Opcode::ADD | Opcode::MUL | Opcode::SUB => {
            ...
            instruction += " ";
            if imm_flag == 1 {
                let imm = u32::from_str_radix(imm_str.trim_start_matches("0x"), 16)?;
